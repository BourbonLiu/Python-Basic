...
def data_list():
    ...
    query = db.select(table_customers).limit(
    ...
    if request.method == "POST":
        try:
            ...
            if request.form['FirstName']:  # 希望至少要填寫名子
                query = db.update(table_customers).where(table_customers.c.CustomerId == request.form['CustomerId']).values(
